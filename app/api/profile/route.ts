...
export async function GET() {
  try {
    ...
    if (rows.length === 0) {
      // Seed default row
      await sql`
        INSERT INTO profile_settings (id, name, email, phone, whatsapp, github, linkedin, facebook, instagram, location, bio, title)
        VALUES (
          1,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const facebook = body.facebook ?? DEFAULT_PROFILE.facebook;
    const instagram = body.instagram ?? DEFAULT_PROFILE.instagram;
    ...
    await sql`
      INSERT INTO profile_settings (id, name, email, phone, whatsapp, github, linkedin, facebook, instagram, location, bio, title, updated_at)
      VALUES (1, ${name}, ${email}, ${phone}, ${whatsapp}, ${github}, ${linkedin}, ${facebook}, ${instagram}, ${location}, ${bio}, ${title}, CURRENT_TIMESTAMP)
    ...
    return new Response(
      JSON.stringify({
        ...
        data: { name, email, phone, whatsapp, github, linkedin, facebook, instagram, location, bio, title },
      }),
      {
        status: 200,
        headers: { "Content-Type": "application/json" },
